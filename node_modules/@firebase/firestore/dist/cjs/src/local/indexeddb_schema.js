...
function createOrUpgradeDb(db, txn, fromVersion, toVersion) {
    ...
    // counting).
    assert_1.assert(fromVersion < toVersion && fromVersion >= 0 && toVersion <= 2, 'Unexpected schema upgrade from v${fromVersion} to v{toVersion}.');
    ...
    var p = persistence_promise_1.PersistencePromise.resolve();
...
function ensureTargetGlobalExists(txn) {
    ...
    return globalStore.get(DbTargetGlobal.key).next(function (metadata) {
        if (metadata != null) {
            return persistence_promise_1.PersistencePromise.resolve(metadata);
        }
        else {
